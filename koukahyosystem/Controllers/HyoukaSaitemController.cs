...
namespace koukahyosystem.Controllers
{
    public class HyoukaSaitemController : Controller
    {
        ...
        public  string firsthyoukacode;
        DataTable dt_hyoukaques = new DataTable();
        ...
        public ActionResult HyoukaSaitem(string id)
        {
            ...
            if (Session["isAuthenticated"] != null)
            {
                ...
                try
                {
                    ...
                    hk.HyokashaList = shainList(jiki, kubun);
                    hk.hyoukacode = firsthyoukacode;
                    if (hk.hyokalist.Count() > 0)
                    {
        ...
        #region get_shainlist()
        private IEnumerable<SelectListItem> shainList(string jiki,string kubun)
        {
            ...
            var mysqlcontroller = new SqlDataConnController();
                ...
                logid = FindLoginId(name);
                 ...
                 string hyoukasql = "SELECT ms.cSHAIN,ms.sSHAIN,mh.cIRAISHA, " +
                                     " fHYOUKA FROM r_hyouka as mh" +
                    ...
                    var readData1 = new SqlDataConnController();
                        ds = readData1.ReadData(hyoukasql);
                foreach (DataRow dr in ds.Rows)
                {
                    string hyoukacheck = "";
                    if(dr["fHYOUKA"].ToString()=="1")
                    {
                        hyoukacheck = "済・";
                    }
                    shainlist.Add(new SelectListItem
                    {
                        ...
                        Text = hyoukacheck+dr["sSHAIN"].ToString()
                    });
                    //if (shainlist.Count != 0)
                    //{
                    //    firsthyoukacode = shainlist[0].Value.ToString();
                    //}
                }
        ...
        public ActionResult HyoukaSaitem(string date, string id, string jiki, string kubun, Models.HyoukaSaitem shain)
         {
            ...
            if (Session["isAuthenticated"] != null)
            {
                ...
                try
                {
                    ...
                    if(Request["btniraiselect"] != null)
                    {
                        firsthyoukacode = Request["selectiraicode"];
                    }
                    else
                    {
                        //firsthyoukacode = Request["selectiraicode"];
        ...
        private Models.HyoukaSaitem GetAllEmployees(string jiki, string kubun)
        {
            Models.HyoukaSaitem model = new Models.HyoukaSaitem();
            var empmodel = new List<Models.hyokalist>();
            var mysqlcontroller = new SqlDataConnController();
            try
            {
                ...
                try
                {
                    DataTable dt_hyoukaques1 = new DataTable();
                    DataTable dt_hyouka1 = new DataTable();
                    ...
                    string komoku = "set @row_num=0;select  @row_num:=@row_num+1 as qno,s.sKOUMOKU as 質問事項 " +
                                    "  FROM r_hyouka as m" +
                    ...
                    dt_hyoukaques1 = readData.ReadData(komoku);//20210122 update retrieve sql
                                                              // dt_hyouka.Columns.Add("qno", typeof(String)).SetOrdinal(0);
                    dt_hyouka1.Columns.Add("qno", typeof(String)).SetOrdinal(0);
                    dt_hyouka1.Columns.Add("質問事項", typeof(String)).SetOrdinal(1);
                    ...
                    #region iraicode
                    string iraisha = "";
                    if (firsthyoukacode == "" || firsthyoukacode==null)
                    {
                        string hyoukacheck = "";
                        hyoukacheck = "and fHYOUKA!= '1' group by mh.cIRAISHA";
                        string hyoukaquery = "SELECT ms.cSHAIN,ms.sSHAIN,cIRAISHA FROM r_hyouka as mh" +
                                            " inner join m_shain as ms on ms.cSHAIN = mh.cIRAISHA " +
                                            " where mh.cKUBUN='" + kubun + "'  and mh.cHYOUKASHA='" + logid + "' " +
                                            " and mh.nJIKI ='" + jiki + "' and mh.dNENDOU='" + Year + "'   ";

                        string hyoukaquery_0 = hyoukaquery + hyoukacheck;
                        DataTable dshyouka = new DataTable();
                        
                        dshyouka = readData1.ReadData(hyoukaquery_0);
                        if (dshyouka.Rows.Count > 0)
                        {
                            foreach (DataRow dr1 in dshyouka.Rows)
                            {
                                iraisha = dr1["cIRAISHA"].ToString();
                                firsthyoukacode = iraisha;
                                break;
                        ...
                        {
                            hyoukacheck = "";
                            hyoukacheck = " group by mh.cIRAISHA";
                            string hyoukaquery_all = hyoukaquery + hyoukacheck;
                            dshyouka = new DataTable();
                            dshyouka = readData1.ReadData(hyoukaquery_all);
                            if (dshyouka.Rows.Count > 0)
                            {
                                foreach (DataRow dr1 in dshyouka.Rows)
                                {
                   ...
                   // string iraisha = "0033";
                    ...
                    if (ds.Rows.Count > 0)
                    {
                        ...
                        string ntenquery = "SELECT count(fHYOUKA) FROM r_hyouka where cHYOUKASHA='" + logid + "' and cIRAISHA='" + iraisha + "' and nJIKI='" + jiki + "'" +
                        ...
                        string query = "select m.nTEN " +
                                      ...
                                      " where s.cKUBUN='" + kubun + "' and m.cIRAISHA ='" + iraisha + "' " +
                        ...
                        DataRow row = dt_hyouka1.NewRow();
                        foreach (DataRow dr1 in dt_hyoukaques1.Rows)
                        {
                            if (ncount != "0")
                            {
                                empmodel.Add(new Models.hyokalist
                                {

                                    No = dr1["qno"].ToString(),
                                    ques = dr1["質問事項"].ToString(),
                                    nTen = ds2.Rows[k]["nTEN"].ToString(),
                                    fHYOUKA = "1",

                                });
                                model.btn_disabled = "disabled";
                            }
                            else
                            {
                                empmodel.Add(new Models.hyokalist
        ...
        private DataTable ReadData(string jiki, string kubun)
        {
            ...
            try
            {
                ...
                var readData = new SqlDataConnController();
                //  dt_hyouka = readData.ReadData(komoku);//20210122 update retrieve sql
                dt_hyoukaques = readData.ReadData(komoku);//20210122 update retrieve sql
                                                          // dt_hyouka.Columns.Add("qno", typeof(String)).SetOrdinal(0);
                dt_hyouka.Columns.Add("qno", typeof(String)).SetOrdinal(0);
                dt_hyouka.Columns.Add("質問事項", typeof(String)).SetOrdinal(1);
                ...
                if (ds.Rows.Count > 0)
                {
                    ...
                    int k = 0;
                    // DataRow dr=new DataRow();
                    DataRow row = dt_hyouka.NewRow();
                    foreach (DataRow dr1 in dt_hyoukaques.Rows)
                    {
                        ...
                        if (ncount != "0")
                        {
                            row = dt_hyouka.NewRow();
                            row["質問事項"] = dr1["質問事項"].ToString();
                            dt_hyouka.Rows.Add(row);
                            row = dt_hyouka.NewRow();
                            row["質問事項"] = ds2.Rows[k]["nTEN"].ToString();
                            dt_hyouka.Rows.Add(row);
                            //string a = dr1["質問事項"].ToString();
                            // dt_hyouka.Rows[i]["質問事項"] = dr1["質問事項"].ToString();
                            //dt_hyouka.Rows[i + 1]["質問事項"] = ds2.Rows[k];
                            // dt_hyouka.Rows[i][dr["cIRAISHA"].ToString()] = dr1["nTEN"].ToString();
                            //dt_hyouka.Rows[i]["qno"] = i + 1;
                            dt_hyouka.Rows[i]["qno"] = dr1["qno"].ToString();
                        }
                        else
                        {
        ...
        private DataTable ReadData1(string jiki, string kubun)
        {
        ...
        [HttpPost]
        public ActionResult kanryou_btn_clcik(string itemlist, string txtyear,string iraicode)
        {
            if (Session["isAuthenticated"] != null)
            {
                ...
                try
                {
                    ...
                    int ntemcount = dlistempty.Rows.Count;
        ...
        [HttpPost]
        public ActionResult kanryou_btn_save(string itemlist, string txtyear,string iraicode)
        {
            if (Session["isAuthenticated"] != null)
            {
                string result = String.Empty;
                try
                {
                    ...
                    string cc = dt.Rows.Count.ToString();
                    double rkvalue = 1;
                    try
                    {
        ...
        [HttpPost]
        public ActionResult Ichijihozon_btn_clcik(string itemlist2, string txtyear,string iraicode)
        {
            if (Session["isAuthenticated"] != null)
            {
                //Year = Session["date"].ToString();
