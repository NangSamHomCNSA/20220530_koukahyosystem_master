...
namespace koukahyosystem.Controllers
{
    public class JyuyouTaskController : Controller
    {
        ...
        {
            string type_year = get_saitehouhouYear(kubun, year);
            var myqlController = new SqlDataConnController();
            DataTable dt_percentage = new DataTable();
            string percent_query = string.Empty;
            percent_query = "SELECT nUPPERLIMIT, nLOWERLIMIT FROM m_saitenhouhou" +
        ...
        public string get_tantoushabusho_1(string logid_Name)
        {
            ...
            try
            {
                ...
                //string hyoukaQuery = "SELECT cSHAIN FROM m_shain where cHYOUKASHA='" + logid_Name + "';";
                //DataTable dthyouka = new DataTable();
                //dthyouka = mysqlcontroller.ReadData(hyoukaQuery);
                ...
                //    allhyouka = allhyouka.Remove(allhyouka.Length - 1, 1);
                //}
                allhyouka = hyoukalist(logid_Name);
            }
            catch
            {
        ...
        public string get_firstshaincode(string busho_Name)
        {
            ...
            try
            {
                ...
                #region hyoukalist
                string allhyouka = "";
                //string hyoukaQuery = "SELECT cSHAIN FROM m_shain where cHYOUKASHA='" + logid + "';";
                //DataTable dthyouka = new DataTable();
                //dthyouka = mysqlcontroller.ReadData(hyoukaQuery);
                //string allhyouka = "";
                //foreach (DataRow Lsdr in dthyouka.Rows)
                ...
                try
                {
                    ...
                    {
                        sqlStr = "SELECT ms.cSHAIN,ms.sSHAIN FROM m_shain as ms inner join r_jishitasuku as r on r.cSHAIN=ms.cSHAIN  " +
                                         " where    r.cSHAIN in(" + allhyouka + ")  " + // 20220519 added by lwin mar
                                       //" where    r.cKAKUNINSHA in('" + logid + "')  " +
