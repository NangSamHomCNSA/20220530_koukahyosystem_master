...
namespace koukahyosystem.Controllers
{
    public class SaitenkakuninTabController : Controller
    {
        ...
        int iraicount = 0;//20220516 ルインマー added check 360seigen
        #endregion
        ...
        private List<Models.saitentable_lists> saitenTableValues(string login_id, string kubun_code, string year)
        {
            ...
            hyoukasha_count = IraishaCount(pg_year,login_id);//20220511 ルインマー added check 360seigen;//get hyoukasha count in login id
        ...
        #endregion
        #region  360seigenからデータを読み取る
        private int IraishaCount(string year,string logid)
        {
            ...
            if (Year != "")
            {
                ...
                DataTable dt_shain = mysqlcontroller.ReadData(mysqlstring);

                for (int i = 0; i < dt_shain.Rows.Count; i++)
                {
                    if (dt_shain.Rows[i]["nseigen"].ToString() != "")
                    {
                        iraicount =Convert.ToInt32(dt_shain.Rows[i]["nseigen"].ToString());
                    }
                }
            }
            string jikiQuery = "SELECT count(*) as COUNT FROM r_hyouka where cIRAISHA='"+logid+"' and " +
                               " fHYOUKA=1 and dNENDOU='" + year + "';";

            System.Data.DataTable dt_jiki = new System.Data.DataTable();
            mysqlcontroller = new SqlDataConnController();
            dt_jiki = mysqlcontroller.ReadData(jikiQuery);
            if (iraicount == 0 && dt_jiki.Rows.Count>0)
            {
                iraicount = 10;
            }
